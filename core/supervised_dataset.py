...
with open("./prompts/jamira_input_prompt.txt", "r") as f:
    prompt = f.read()


def format_input(st , prompt):
    prompt = prompt.format(conclusion=st)
    return prompt

def format_output(st):
    return f"""The label is {st}"""
...
def preprocess(train_on_inputs: bool , samples: Sequence[str] , tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    sources = [f"{format_input(st , prompt)}" for st in samples['conclusion']]
    targets = [f"{format_output(st)}{tokenizer.eos_token}" for st in samples['label']]
